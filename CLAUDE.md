...
Recent cleanup efforts focused on aligning code with the multi-format document support and removing deprecated PDF-specific code:

**Files Removed:**
1. `api/routes.py` - Completely unused, redundant with routes in `main.py`, contained outdated schemas
2. `services/pdf_extractor.py` - Superseded by `DocumentExtractor` which supports PDF, TXT, DOCX, CSV

**Files Updated:**
1. `config.py` - Fixed directory creation from `data/pdfs/` to `data/documents/`
2. `verify_setup.py` - Updated to check `DocumentExtractor` instead of `PDFExtractor`

**Files Reorganized:**
1. Desktop icon utilities moved to `desktop/utils/` for better organization

**Directory Structure Clarification:**
- JSON storage: `data/indexes/json/` (contains `faiss.index`, `metadata.json`, `embeddings.npy`)
- SQLite storage: `data/indexes/sqlite/` (contains `faiss.index`, `metadata.db`)
- Both storage types use separate subdirectories to prevent conflicts when switching
